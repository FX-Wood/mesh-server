...
module.exports.updateReward = async (meshId,liked) => {
    try {
        logger.log('info', 'updateReward-inputs', { data: { meshId, liked }})
        ...
        logger.log('info', 'updateReward-outputs', { data: logData })
    } catch(err) {
        logger.log('error', 'updateReward-error', { data: {error: err.message, stack: err.stack} })
    }
}
...
const chooseNextMesh = async () => {
    ...
    if (rand > EPSILON) {
        ...
        logger.log('info', 'chooseNextMesh explore', { data: logData })
    } else {
        ...
        logger.log('info', 'chooseNextMesh exploit', { data: logData })
    }
    ...
    if (nextCluster) {
        ...
        logger.log('info', 'chooseNextMesh success', { data: logData })
        return nextMesh
    } else {
        const logData = {
            epsilon: EPSILON,
        ...
        logger.log('info', 'chooseNextMesh failure', { data: logData })
        return undefined
    }
}
